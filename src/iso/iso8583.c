...
static const char* getMTI(void) { return __iso8583->mti; }

/* ===== Fields ===== */

IsoStatus_t setStr(uint16_t field, const void* data) {
    RETURN_VALUE_IF_NULL(data, ;, ISO_ERR_INPUT);
    SELF;
    if (field == 0 || field > ISO_MAX_FIELDS)
        return ISO_ERR_INVALID_FIELD;

    if (DL_ISO8583_MSG_SetField_Str(field, (const uint8_t*)data, &self->msg) !=
        0)
        return ISO_ERR_PACK;
...
IsoStatus_t setBin(uint16_t field, const void* data, size_t len) {
    RETURN_VALUE_IF_NULL(data, ;, ISO_ERR_INPUT);
    SELF;
    if (field == 0 || field > ISO_MAX_FIELDS)
        return ISO_ERR_INVALID_FIELD;

    if (DL_ISO8583_MSG_SetField_Bin(field, (const uint8_t*)data, len,
                                    &self->msg) != 0)
        return ISO_ERR_PACK;
...
IsoStatus_t getStr(uint16_t field, void* out) {
...
IsoStatus_t getBin(uint16_t field, void* out, size_t* len) {
    RETURN_VALUE_IF_NULL(out, ;, ISO_ERR_INPUT);
...
IsoStatus_t pack(const uint8_t* data, size_t* outlen) {
    SELF;
    RETURN_VALUE_IF_NULL(data, ;, ISO_ERR_INPUT);
    ...
    RETURN_VALUE_IF_NOT(
        DL_ISO8583_MSG_SetField_Str(0, (const uint8_t*)self->mti, &self->msg),
        0,
        ;, ISO_ERR_PACK);

    int out_len = 0;
    RETURN_VALUE_IF_NOT(
        DL_ISO8583_MSG_Pack(&self->handler, &self->msg, data, &out_len), 0, ;
        , ISO_ERR_PACK);
...
IsoStatus_t parse(const uint8_t* data, size_t len) {
    SELF;
    RETURN_VALUE_IF_NULL(data, ;, ISO_ERR_INPUT);
    reset();
#ifdef USE_DUMP
    ...
    DL_ISO8583_MSG_Unpack(&self->handler, packBufTemp, packedSize, &self->msg);
...
Iso8583* iso8583() {
    CALL_ONCE(__iso8583 = MEM_ALLOC(sizeof(Iso8583));
