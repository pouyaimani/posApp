...
#define DL_STR_SafeStr(str) ((str) == NULL ? kDL_STR_EmptyStr : (str))

/* accepts a pointer to a numeric - outputs safe values (ie 0 if NULL) */
#define DL_STR_SafeNum(a) ((a) == NULL ? 0 : *(a))
...
#define DL_STR_IsWS(iCh)                                                       \
    ((((iCh) >= kDL_ASCII_HT) && ((iCh) <= kDL_ASCII_CR)) ||                   \
     ((iCh) == kDL_ASCII_SP))
...
/* NB if 'iStr' is NULL then empty string is output  */
/* returns: error code                               */
DL_ERR DL_STR_StrNDup(const DL_CHAR* iStr, DL_UINT32 iMaxChars, DL_CHAR** oStr);
