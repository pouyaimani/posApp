...
static void DL_TIME_Populate(int iYear, int iMonth, int iDay, int iHour,
                             int iMinute, int iSecond, int _iDayOfWeek,
                             DL_TIME* oData);

/******************************************************************************/

static void seconds_to_struct(DL_UINT32 iSeconds, DL_TIME* oData);

/******************************************************************************/

/* NB '_year' should be specified if 'day-in-month' mode used */
static void struct_to_timestamp(int _iYear, const DL_TIME* iData,
                                char* oTimestampStr);

/******************************************************************************/

/* NB '_year' should be specified if 'day-in-month' mode used */
static void struct_to_seconds(int _iYear, const DL_TIME* iData,
                              DL_UINT32* oSeconds);

/* converts a timestamp (YYYYMMDDHHMISS) to the date/time structure */
/* returns: 1 if ok / 0 otherwise (eg invalid timestamp)            */
static int timestamp_to_struct(const char* iTimestampStr, DL_TIME* oData);

/******************************************************************************/

// ensures that any 'Day-in-Month' values are converted to absolute
static void rationalise_struct(int _iYear, const DL_TIME* iData,
                               DL_TIME* oData);

static int calc_day_of_week(int iYear, int iMonth, int iDay);

static int calc_days_in_month(int iYear, int iMonth);

static void calc_dst_start_end(int _iYear, DL_UINT32* iDstStart,
                               DL_UINT32* iDstEnd);
...
void DL_TIME_ConvUTCSecondsToLocalStruct(DL_UINT32 iUtcSecs, DL_TIME* oData) {
    ...
    if (TZ_INFO_HAS_DST(&_tz)) {
        ...
        if (((dstStart <= dstEnd) && (iUtcSecs >= dstStart) &&
             (iUtcSecs < dstEnd)) ||
            ((dstStart > dstEnd) &&
             ((iUtcSecs >= dstStart) || (iUtcSecs < dstEnd)))) {
            inDST = 1;
        }
    ...
    *oUtcSecs = 0;
    ...
    if (ok) {
        ...
        if (TZ_INFO_HAS_DST(&_tz)) {
            ...
            if (((dstStart <= dstEnd) && (*oUtcSecs >= dstStart) &&
                 (*oUtcSecs < dstEnd)) ||
                ((dstStart > dstEnd) &&
                 ((*oUtcSecs >= dstStart) || (*oUtcSecs < dstEnd)))) {
                inDST = 1;
            }
...
void DL_TIME_ConvUTCSecondsToLocalFormatStr(DL_UINT32 iUtcSecs,
    ...
    /* populate buffer */
    sprintf(ioFormatStr, "%02d/%02d/%04d (%02d:%02d:%02d)", st.day, st.month,
            st.year, st.hour, st.minute, st.second);
...
static void _get_tz_info(void) {
...
#if defined(DL_WIN32)
    {
        ...
        if (tempOk) {
            ...
            /* DST start */
            DL_TIME_Populate(
                winTZ.DaylightDate.wYear, winTZ.DaylightDate.wMonth,
                winTZ.DaylightDate.wDay, winTZ.DaylightDate.wHour,
                winTZ.DaylightDate.wMinute, winTZ.DaylightDate.wSecond,
                winTZ.DaylightDate.wDayOfWeek, &(_tz.dstStart));

            /* DST end */
            DL_TIME_Populate(
                winTZ.StandardDate.wYear, winTZ.StandardDate.wMonth,
                winTZ.StandardDate.wDay, winTZ.StandardDate.wHour,
                winTZ.StandardDate.wMinute, winTZ.StandardDate.wSecond,
                winTZ.StandardDate.wDayOfWeek, &(_tz.dstEnd));
        }
...
static void seconds_to_struct(DL_UINT32 iSeconds, DL_TIME* oData) {
    ...
    int year = kMIN_YEAR, month = 1, day = 1, hour = 0, minute = 0, second = 0,
        DoW = 0;
    ...
    do {
        ...
        tempUInt32 =
            isLeapYear(year) ? kSECONDS_IN_LEAP_YEAR : kSECONDS_IN_NORMAL_YEAR;
...
static void struct_to_timestamp(int _iYear, const DL_TIME* iData,
    ...
    sprintf(oTimestampStr, "%04d%02d%02d%02d%02d%02d", tmpSt.year, tmpSt.month,
            tmpSt.day, tmpSt.hour, tmpSt.minute, tmpSt.second);
...
static int timestamp_to_struct(const char* iTimestampStr, DL_TIME* oData) {
    ...
    ok = DL_STR_Validate(iTimestampStr, kDL_TIME_TIMESTAMP_LEN,
                         kDL_TIME_TIMESTAMP_LEN, "0123456789");
    ...
    if (ok) {
        ...
        sscanf(iTimestampStr, "%04d%02d%02d%02d%02d%02d", &year, &month, &day,
               &hour, &minute, &second);
    }
    ...
    if (ok) {
        DL_TIME_Populate(year, month, day, hour, minute, second,
                         calc_day_of_week(year, month, day), oData);
    }
...
static void rationalise_struct(int _iYear, const DL_TIME* iData,
    ...
    if (iData->year == 0) {
        ...
        {
            oData->day =
                (((calc_days_in_month(_iYear, iData->month) - first) / 7) * 7) +
                first;
