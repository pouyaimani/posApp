...
// returns: error code
DL_ERR DL_ISO8583_MSG_SetField_Str(DL_UINT16 iField, const DL_UINT8* iDataStr,
                                   DL_ISO8583_MSG* ioMsg) {
    DL_ERR err = kDL_ERR_NONE;

    err = DL_ISO8583_MSG_SetField_Bin(
        iField, iDataStr, (DL_UINT16)DL_STR_StrLen(iDataStr), ioMsg);
...
// returns: error code
DL_ERR DL_ISO8583_MSG_SetField_Bin(DL_UINT16 iField, const DL_UINT8* _iData,
                                   DL_UINT16 _iDataLen, DL_ISO8583_MSG* ioMsg) {
    DL_ERR    err = kDL_ERR_NONE;
    DL_UINT8* ptr = NULL;
...
int DL_ISO8583_MSG_HaveField(DL_UINT16 iField, const DL_ISO8583_MSG* iMsg) {
    if ((NULL != iMsg) && (iField <= kDL_ISO8583_MAX_FIELD_IDX) &&
...
// returns: error code
DL_ERR DL_ISO8583_MSG_GetField_Str(DL_UINT16 iField, const DL_ISO8583_MSG* iMsg,
                                   DL_UINT8** oPtr) {
...
// returns: error code
DL_ERR DL_ISO8583_MSG_GetField_Bin(DL_UINT16 iField, const DL_ISO8583_MSG* iMsg,
                                   DL_UINT8** oPtr, DL_UINT16* oByteLen) {
    DL_ERR err = kDL_ERR_OTHER; /* assume error */
...
DL_ERR DL_ISO8583_MSG_Pack(const DL_ISO8583_HANDLER* iHandler,
                           const DL_ISO8583_MSG* iMsg, DL_UINT8* ioByteArr,
                           DL_UINT16* oNumBytes) {
    DL_ERR    err    = kDL_ERR_NONE;
    ...
    *oNumBytes = 0;
    ...
    for (fieldIdx = 0; (fieldIdx < MIN(iHandler->fieldItems,
                                       kDL_ISO8583_MAX_FIELD_IDX + 1)) &&
                       !err;
         fieldIdx++) {
        if ((NULL != iMsg->field[fieldIdx].ptr) || // present
            DL_ISO8583_IS_BITMAP(
                iHandler->fieldArr[fieldIdx].fieldType)) // bitmap
        {
            /* pack field */
...
DL_ERR DL_ISO8583_MSG_Unpack(const DL_ISO8583_HANDLER* iHandler,
                             const DL_UINT8* iByteArr, DL_UINT16 iByteArrSize,
                             DL_ISO8583_MSG* ioMsg) {
    DL_ERR    err         = kDL_ERR_NONE;
    DL_UINT8* curPtr      = (DL_UINT8*)iByteArr;
    DL_UINT8* endPtr      = curPtr + iByteArrSize;
    DL_UINT16 curFieldIdx = 0;
    DL_UINT16 maxFieldIdx =
        MIN(kDL_ISO8583_MAX_FIELD_IDX, (iHandler->fieldItems) - 1);
    int haveBitmap = 0;
...
void DL_ISO8583_MSG_Dump(FILE* iOutFile, const char* _iEolStr,
    ...
    for (i = 0; i < (iHandler->fieldItems); i++) {
        if (NULL != iMsg->field[i].ptr) /* present */
        {
            DL_ISO8583_FIELD_DEF* fieldDef =
                DL_ISO8583_GetFieldDef(i, iHandler);
