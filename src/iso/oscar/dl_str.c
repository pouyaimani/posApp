...
/* returns: n/a                                                               */
static void unescape_string(DL_CHAR iEscCh, DL_CHAR* ioStr);
...
int DL_STR_StrCmp(const DL_CHAR* iStr1, const DL_CHAR* iStr2, int iIgnoreCase) {
...
int DL_STR_StrNCmp(const DL_CHAR* iStr1, const DL_CHAR* iStr2, int iIgnoreCase,
                   int iMaxChars) {
    int retVal = 0;
    int ch1, ch2;
    int charCnt = 0;
...
void DL_STR_StrCpy(DL_CHAR* ioToPtr, const DL_CHAR* iFromPtr, int iMaxChars) {
...
DL_ERR DL_STR_StrDup(const DL_CHAR* iStr, DL_CHAR** oStr) {
    return DL_STR_StrNDup(iStr, DL_STR_StrLen(iStr), oStr);
}

/******************************************************************************/

DL_ERR DL_STR_StrNDup(const DL_CHAR* iStr, DL_UINT32 iMaxChars,
                      DL_CHAR** oStr) {
...
DL_ERR DL_STR_StrCat(const DL_CHAR* iStr1, const DL_CHAR* iStr2,
                     DL_CHAR** oStr) {
    DL_ERR err     = kDL_ERR_NONE;
    int    str1len = 0;
    int    str2len = 0;
...
void DL_STR_LTrim(DL_CHAR* ioStr, DL_CHAR iTrimCh) {
    DL_CHAR* ptr = NULL;
...
void DL_STR_RTrim(DL_CHAR* ioStr, DL_CHAR iTrimCh) {
    DL_CHAR* ptr = NULL;
...
int DL_STR_Validate(const DL_CHAR* iStr, int iMinLen, int iMaxLen,
...
int DL_STR_Contains(const DL_CHAR* iStr, const DL_CHAR* iContains) {
    int ret = 0; /* assume No */
...
DL_ERR DL_STR_EncapsulateStr(const DL_CHAR* iStr, DL_CHAR iEncapChar,
                             DL_CHAR** oStr) {
    DL_ERR   err    = kDL_ERR_NONE;
    DL_CHAR* bufPtr = NULL;
...
DL_ERR DL_STR_GetEncapsulatedStr(const DL_CHAR* iStr, DL_CHAR iEncapChar,
                                 DL_CHAR** oDataStr, DL_CHAR** oNextPtr) {
    DL_ERR   err      = kDL_ERR_NONE;
    ...
    } else /* does match */
    {
        ...
        while (1) {
            if (*endPtr == kDL_ASCII_NULL) /* unexpected end */
            {
                err = kDL_ERR_OTHER;
                break;
            } else if (kDL_ASCII_BACKSLASH ==
                       *endPtr) /* escape sequence (start) */
            {
                ...
                if (*endPtr ==
                    kDL_ASCII_NULL) /* unexpected end (during escape) */
                {
                    err = kDL_ERR_OTHER;
                    break;
                }
...
DL_CHAR* DL_STR_ReadToBuffer(const DL_CHAR* iStr, const DL_CHAR* iValidChars,
                             int iBufferSize, DL_CHAR* oBuffer) {
    DL_CHAR* readPtr   = (DL_CHAR*)iStr;
    int      charsRead = 0;
