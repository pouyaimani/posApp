...
// static uint32_t parameters = EMBEDDB_RECORD_LEVEL_CONSISTENCY |
...
static void extractLatestIdx() { CALL_ONCE(getLatestFirstTime(&latestIdx);); }
...
static int8_t shiftInsert(ShiftData* shift) {
    ...
    LOG_DEBUG("shift: idx = %d, startTime = %d, endTime = %d, startDate = %d, "
              "endDate = %d",
              idx, shift->startTime, shift->endTime, shift->startDate,
              shift->endDate);
    latestIdx++;
    return ERR_OK;
}
...
static uint32_t getLatestIdx() { return latestIdx; }
...
static int8_t keep(ShiftData* data) {
    if (!data)
        return ERR_NOK;
    tmpShift = *data;
    return storage()->save(shiftDesc,
                           sizeof(shiftDesc) / sizeof(DataDescriptor),
                           SHIFTS_RECORD_TMP);
}
...
static int8_t getKeeped(ShiftData* data) {
    if (!data)
        return ERR_NOK;
    if (storage()->load(shiftDesc, sizeof(shiftDesc) / sizeof(DataDescriptor),
                        SHIFTS_RECORD_TMP) != ERR_OK) {
        return ERR_NOK;
    }
...
static int8_t init() {
    ...
    uint32_t parameters = EMBEDDB_RECORD_LEVEL_CONSISTENCY |
                          (EMBEDDB_USE_BMAP | EMBEDDB_USE_INDEX);
    if (embedDBSetup(state, SHIFTS_RECORD_PATH, SHIFTS_RECORD_IDX_PATH,
                     sizeof(uint32_t), sizeof(ShiftData), PAGE_SIZE_512,
...
Shifts* shifts() {
    CALL_ONCE(OOP_CALL_CTOR(Shifts, &__shifts););
    return &__shifts;
}