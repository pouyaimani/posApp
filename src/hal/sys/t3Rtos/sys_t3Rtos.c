...
const core_app_start app_entry
    __attribute__((at(APP_CORE_START + MCU_BASE_ADDR))) = appMain;
// end
...
static void initRam(void) {
    ...
    unsigned int* end =
        src + ((unsigned int)&Image$$RW_IRAM1$$Length / sizeof(unsigned int));
...
static void parseRtcTime(const uint8_t* rtcTime, int* year, int* month,
...
static int8_t getSN(System* dev, char* out, size_t len) {
    ...
    //                  out, len);
    // if (err != SDK_SYS_OK) {
    //     LOG_ERROR("SYS: failure in reading device serail number. error = %d",
    //     err); return ERR_NOK;
    // }
