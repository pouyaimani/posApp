...
typedef enum {
    FILE_SEEK_SET = 0,  /* From beginning of file */
    FILE_SEEK_CUR = 1,  /* From current position */
    FILE_SEEK_END = 2   /* From end of file */
} FileSeekOrigin_t;
...
OOP_VTABLE(File) {
    ...
    OOP_IMETHOD(size_t, File, read, void* buffer, size_t size, size_t count, FileHandle* handle);
    ...
    OOP_IMETHOD(size_t, File, write, const void* buffer, size_t size, size_t count, FileHandle* handle);
     ...
     * Returns: 0 on success, non-zero on failure */
    OOP_IMETHOD(int, File, seek, FileHandle* handle, long offset, FileSeekOrigin_t origin);
