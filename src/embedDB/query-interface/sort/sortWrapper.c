...
#endif
        file_iterator_state_t *iteratorState = EMDB_MEM_ALLOC(sizeof(file_iterator_state_t));
...
file_iterator_state_t *startSort(sortData *data, void *unsortedFile, void *sortedFile) {
    ...
    char *buffer = EMDB_MEM_ALLOC(buffer_max_pages * es.page_size + es.record_size);
