 ...
 * memory
 */
file_iterator_state_t* startPureMemorySort(sortData*        data,
                                           embedDBOperator* op) {
#ifdef DEBUG
    ...
    if (record_count == 0) {
        ...
        file_iterator_state_t* iteratorState =
            EMDB_MEM_ALLOC(sizeof(file_iterator_state_t));
    ...
    void* buffer = EMDB_MEM_ALLOC(record_count * data->recordSize);
    if (buffer == NULL) {
#ifdef PRINT_ERRORS
    ...
    while (exec(op->input) && records_read < record_count) {
        memcpy((char*)buffer + records_read * data->recordSize,
               op->input->recordBuffer, data->recordSize);
        records_read++;
    }
    ...
    metrics_t metrics = {0};
    int       sort_result =
        in_memory_quick_sort(buffer, records_read, data->recordSize,
                             data->keyOffset, data->compareFn, &metrics);

    if (sort_result != 0) {
#ifdef PRINT_ERRORS
        debug_log("ERROR: In-memory sort failed\n");
#endif
        EMDB_MEM_FREE(buffer);
        return NULL;
    }
 ...
 *
 * @param buffer            The buffer that is written. Should be atleast the
 * size of pageSize
 ...
 * @return int8_t
 */
int8_t writePageWithHeader(void* buffer, const uint32_t blockIndex,
                           const uint16_t              numberOfValues,
                           const uint32_t              pageSize,
                           const embedDBFileInterface* fileInterface,
                           void*                       file) {
...
uint32_t loadRowData(sortData* data, embedDBOperator* op, void* unsortedFile) {
...
#ifdef DEBUG
    debug_log("DEBUG loadRowData: PAGE_SIZE=%d, BLOCK_HEADER_SIZE=%d, "
              "recordSize=%d, valuesPerPage=%d\n",
              PAGE_SIZE, BLOCK_HEADER_SIZE, data->recordSize, valuesPerPage);
#endif

    void* buffer = EMDB_MEM_ALLOC(PAGE_SIZE);

    if (buffer == NULL) {
#ifdef PRINT_ERRORS
        debug_log("ERROR: SORT: buffer EMDB_MEM_ALLOC failed");
#endif
        return 1;
    }
    ...
    while (exec(op->input)) {
        ...
        if (count % valuesPerPage == 0 && count != 0) {
            if (writePageWithHeader(buffer, blockIndex, valuesPerPage,
                                    PAGE_SIZE, data->fileInterface,
                                    unsortedFile)) {
                EMDB_MEM_FREE(buffer);
                buffer = NULL;
                return 0;
        ...
        uint32_t rowOffset =
            (count % valuesPerPage) * data->recordSize + BLOCK_HEADER_SIZE;
        ...
        memcpy((uint8_t*)buffer + rowOffset, op->input->recordBuffer,
               data->recordSize);
#ifdef DEBUG
        if (count < 10) {
        ...
        if (count < 10 || count % 1000 == 0) {
            int32_t* keyPtr =
                (int32_t*)(op->input->recordBuffer + data->keyOffset);
    ...
    uint32_t numRemainingRecords = (count % valuesPerPage == 0 && count != 0)
                                       ? valuesPerPage
                                       : count % valuesPerPage;
    if (writePageWithHeader(buffer, blockIndex, numRemainingRecords, PAGE_SIZE,
                            data->fileInterface, unsortedFile)) {
        EMDB_MEM_FREE(buffer);
        buffer = NULL;
        return 0;
    }
...
void prepareSort(embedDBOperator* op) {
...
#ifdef DEBUG
    debug_log("DEBUG prepareSort: recordSize=%d, keySize=%d, keyOffset=%d, "
              "colNum=%d\n",
...
#ifdef PRINT_ERRORS
        debug_log("ERROR: File interface or setup function not provided while "
                  "initializing ORDER BY operator\n");
#endif
        return;
    }
...
#ifdef PRINT_ERRORS
        debug_log("ERROR: Failed to allocate file handles while initializing "
                  "ORDER BY operator\n");
#endif
        return;
    }
    const uint8_t unsortedOpen =
        data->fileInterface->open(unsortedFile, EMBEDDB_FILE_MODE_W_PLUS_B);
    const uint8_t sortedOpen =
        data->fileInterface->open(sortedFile, EMBEDDB_FILE_MODE_W_PLUS_B);

    if (!unsortedOpen || !sortedOpen) {
#ifdef PRINT_ERRORS
        debug_log(
            "ERROR: Failed to open files while initializing ORDER BY operator");
#endif
        return;
    }
    ...
    // Start sorting
    file_iterator_state_t* iteratorState =
        startSort(data, unsortedFile, sortedFile);
    if (iteratorState == NULL) {
#ifdef PRINT_ERRORS
...
file_iterator_state_t* startSort(sortData* data, void* unsortedFile,
    ...
    es.num_pages   = (uint32_t)ceil(
        (float)data->count / ((es.page_size - es.headerSize) / es.record_size));

    const int buffer_max_pages = 4;

    char* buffer =
        EMDB_MEM_ALLOC(buffer_max_pages * es.page_size + es.record_size);
    char* tuple_buffer = buffer + es.page_size * buffer_max_pages;

    if (buffer == NULL) {
#ifdef PRINT_ERRORS
        debug_log("ERROR: SORT: buffer EMDB_MEM_ALLOC failed m\n");
#endif
        return NULL;
    }
    ...
    if (iteratorState == NULL) {
#ifdef PRINT_ERRORS
        debug_log("Error: SORT: iterator EMDB_MEM_ALLOC failed\n");
#endif
        EMDB_MEM_FREE(buffer);
        buffer = NULL;
        return NULL;
    ...
    err =
        adaptive_sort(readNextRecord, iteratorState, tuple_buffer, sortedFile,
                      buffer, buffer_max_pages, &es, &result_file_ptr, &metrics,
                      data->compareFn, runGenOnly, writeReadRatio, data);

#ifdef PRINT_METRIC
    debug_log(
        "\tComplete. Comparisons: %d  Writes: %d  Reads: %d Memcpys: %d\n",
        metrics.num_compar, metrics.num_writes, metrics.num_reads,
        metrics.num_memcpys);
#endif
...
uint8_t readNextRecord(void* data, void* buffer) {
    ...
    uint32_t recordPerPage =
        (PAGE_SIZE - BLOCK_HEADER_SIZE) / iteratorState->recordSize;

    // Read next page if current buffer is empty
    if (iteratorState->currentRecord % recordPerPage == 0 ||
        iteratorState->recordsRead == 0) {
        uint32_t seekOffset =
            iteratorState->resultFile +
            (iteratorState->currentRecord / recordPerPage) * PAGE_SIZE;

        iteratorState->fileInterface->seek(seekOffset, iteratorState->file);
        iteratorState->fileInterface->readRel(
            ((sortData*)data)->readBuffer, PAGE_SIZE, 1, iteratorState->file);

#ifdef DEBUG
        if (iteratorState->recordsRead == 0 ||
            iteratorState->recordsRead % 1000 == 0) {
            debug_log("DEBUG readNextRecord: pageNum=%d, seekOffset=%d, "
                      "recordsRead=%d\n",
                      iteratorState->currentRecord / recordPerPage, seekOffset,
    ...
    uint32_t copyOffset =
        BLOCK_HEADER_SIZE + iteratorState->recordSize * recordIndexInPage;
    memcpy(buffer, ((sortData*)data)->readBuffer + copyOffset,
           iteratorState->recordSize);

#ifdef DEBUG
    if (iteratorState->recordsRead < 10 ||
        iteratorState->recordsRead % 1000 == 0) {
        int32_t* keyPtr = (int32_t*)(buffer + ((sortData*)data)->keyOffset);
        debug_log("DEBUG readNextRecord: recordsRead=%d, currentRecord=%d, "
                  "pageIdx=%d, recordInPage=%d, copyOffset=%d, key=%d\n",
                  iteratorState->recordsRead, iteratorState->currentRecord,
        ...
        int32_t* key0 =
            (int32_t*)(((sortData*)data)->readBuffer + BLOCK_HEADER_SIZE +
                       ((sortData*)data)->keyOffset);
        int32_t* keyLast =
            (int32_t*)(((sortData*)data)->readBuffer + BLOCK_HEADER_SIZE +
                       (recordPerPage - 1) * iteratorState->recordSize +
                       ((sortData*)data)->keyOffset);
    ...
    //         for (int i = 0; i < iteratorState->recordSize - SORT_KEY_SIZE;
    //         i++) {
