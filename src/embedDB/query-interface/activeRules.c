...
activeRule* createActiveRule(embedDBSchema* schema, void* context) {
    activeRule* rule = (activeRule*)EMDB_MEM_ALLOC(sizeof(activeRule));
    if (rule != NULL) {
...
float GetAvg(embedDBState* state, activeRule* rule, void* key) {
    ...
    recordBuffer = NULL;
    for (int i = 0; i < 2; i++) {
        EMDB_MEM_FREE(allocatedValues[i]);
    }
    EMDB_MEM_FREE(allocatedValues);
    return avg;
}
...
embedDBOperator* createOperator(embedDBState* state, activeRule* rule,
    ...
    embedDBIterator* it =
        (embedDBIterator*)EMDB_MEM_ALLOC(sizeof(embedDBIterator));
    if (state->keySize == 4) {
        uint32_t minKeyVal =
            *(uint32_t*)key - (*(uint32_t*)rule->numLastEntries - 1);
        uint32_t* minKeyPtr = (uint32_t*)EMDB_MEM_ALLOC(sizeof(uint32_t));
        if (minKeyPtr != NULL) {
            *minKeyPtr = minKeyVal;
            it->minKey = minKeyPtr;
        }
        ...
        uint64_t minKeyVal =
            *(uint64_t*)key - (*(uint64_t*)rule->numLastEntries - 1);
        uint64_t* minKeyPtr = (uint64_t*)EMDB_MEM_ALLOC(sizeof(uint64_t));
        if (minKeyPtr != NULL) {
            *minKeyPtr = minKeyVal;
            it->minKey = minKeyPtr;
        }
    ...
    case GET_AVG:
        ...
        break;
    case GET_MAX:
        aggFunc = createMaxAggregate(rule->colNum,
                                     rule->schema->columnSizes[rule->colNum]);
        break;
    case GET_MIN:
        aggFunc = createMinAggregate(rule->colNum,
                                     rule->schema->columnSizes[rule->colNum]);
        break;
    default:
#ifdef PRINT_ERRORS
    ...
    embedDBAggregateFunc* aggFuncs =
        (embedDBAggregateFunc*)EMDB_MEM_ALLOC(1 * sizeof(embedDBAggregateFunc));
    aggFuncs[0] = *aggFunc;
    embedDBOperator* aggOp =
        createAggregateOperator(scanOp, groupFunction, aggFuncs, 1);
    aggOp->init(aggOp);

    EMDB_MEM_FREE(aggFunc);

    *allocatedValues              = (void**)EMDB_MEM_ALLOC(2 * sizeof(void*));
...
int8_t groupFunction(const void* lastRecord, const void* record) { return 1; }

void executeComparison(activeRule* rule, void* aggregateValue,
                       Comparator comparator, void* data) {
    int8_t comparisonResult = comparator(aggregateValue, rule->threshold);

    switch (rule->operation) {
    case GreaterThan:
        if (comparisonResult > 0)
            rule->callback(aggregateValue, data, rule->context);
        break;
    case LessThan:
        if (comparisonResult < 0)
            rule->callback(aggregateValue, data, rule->context);
...
void handleGetAvg(embedDBState* state, activeRule* rule, void* key,
                  void* data) {
