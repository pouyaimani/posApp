...
embedDBOperator* createOperator(embedDBState* state, activeRule* rule, void*** allocatedValues, void* key) {
    embedDBIterator* it = (embedDBIterator*)EMDB_MEM_ALLOC(sizeof(embedDBIterator));
    if (state->keySize == 4) {
    ...
    embedDBAggregateFunc* aggFuncs = (embedDBAggregateFunc*)EMDB_MEM_ALLOC(1 * sizeof(embedDBAggregateFunc));
    aggFuncs[0] = *aggFunc;
