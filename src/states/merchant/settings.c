...
static const Phrases_t SettingsItemTxt[SET_ITEM_ALL] = {
    PHRASE_SOUND_SETTINGS, PHRASE_ENERGY_SAVING, PHRASE_RECEIPT_SETTINGS,
    PHRASE_SCR_BRIGHTNESS, PHRASE_TOUCH_STATUS,  PHRASE_DT_UPDATING,
};
...
static void SoundSettings(State* parent) {
    ...
    subSettings[SET_ITEM_SOUND]->vtable.handleKeypad =
        STATE_HANDLE(SoundSettings, KeypadEvent);
}
...
static void EnergySettings(State* parent) {
    ...
    OOP_CALL_CTOR(State, subSettings[SET_ITEM_ENERGY], parent,
                  "energy settings");
...
static void ReceiptSettings(State* parent) {
    ...
    OOP_CALL_CTOR(State, subSettings[SET_ITEM_RECEIPT], parent,
                  "receipt settings");
...
static void ScrLightSettings(State* parent) {
    ...
    OOP_CALL_CTOR(State, subSettings[SET_ITEM_SCR_LIGHT], parent,
                  "receipt settings");
    subSettings[SET_ITEM_SCR_LIGHT]->vtable.enter =
        STATE_ENTER(ScrLightSettings);
    subSettings[SET_ITEM_SCR_LIGHT]->vtable.exit = STATE_EXIT(ScrLightSettings);
    subSettings[SET_ITEM_SCR_LIGHT]->vtable.handleKeypad =
        STATE_HANDLE(ScrLightSettings, KeypadEvent);
}
...
static void TouchSettings(State* parent) {
    ...
    subSettings[SET_ITEM_TOUCH]->vtable.handleKeypad =
        STATE_HANDLE(TouchSettings, KeypadEvent);
}
...
static void DateTimeSettings(State* parent) {
    ...
    OOP_CALL_CTOR(State, subSettings[SET_ITEM_DATE_TIME], parent,
                  "date time settings");
    subSettings[SET_ITEM_DATE_TIME]->vtable.enter =
        STATE_ENTER(DateTimeSettings);
    subSettings[SET_ITEM_DATE_TIME]->vtable.exit = STATE_EXIT(DateTimeSettings);
    subSettings[SET_ITEM_DATE_TIME]->vtable.handleKeypad =
        STATE_HANDLE(DateTimeSettings, KeypadEvent);
}
