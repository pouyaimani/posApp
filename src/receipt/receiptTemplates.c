...
static int8_t receiptSectionTerminalInfo(Receipt *rec) {
    ...
    snprintf(code, sizeof(code), "%s:%s", 
            settings()->terminal.terminalNo, phraseGetDef(PHRASE_DEVICE_CODE));
    RecColumn_t row[] = {
        {code, LV_TEXT_ALIGN_LEFT, 1},
...
static int8_t receiptSectionRefTrace(Receipt *rec, const uint32_t *trace, const uint32_t *refNum) {
    ...
    snprintf(traceStr, sizeof(traceStr), "%s:%d", phraseGetDef(PHRASE_TRACE), *trace);
    DEFINE_STRING(refStr, 64);
    snprintf(refStr, sizeof(refStr), "%s:%d", phraseGetDef(PHRASE_REFERENCE), *refNum);
    RecColumn_t row[] = {
...
static int8_t receiptSectionBalance(Receipt *rec, const uint64_t *balance, const uint64_t *ledger) {
    ...
    snprintf(balanceStr, sizeof(balanceStr), "%s %d", phraseGetDef(PHRASE_RIAL), *balance);
    RecColumn_t row[] = {
    ...
    snprintf(ledgerStr, sizeof(ledgerStr), "%s %d", *ledger, phraseGetDef(PHRASE_RIAL));
    RecColumn_t row1[] = {
...
static int8_t receiptSectionTxnHeader(Receipt *rec, const uint64_t *dateTime, TxnType type) {
    uint32_t date, time;
    ...
    snprintf(header, sizeof(header), "%s-%s-%s", 
                servName, phraseGetDef(PHRASE_CUSTOMER_RECEIPT),
                    dtStr);
    RecColumn_t row[] = {
...
static int8_t receiptSectionAmount(Receipt *rec, const uint64_t *amount) {
    ...
    snprintf(amountStr, sizeof(amountStr), "%s %s", amountStr, phraseGetDef(PHRASE_RIAL));
    RecColumn_t row[] = {
...
static int8_t buildSaleReceipt(Receipt *rec, const ReceiptData *data) {
    ...
    RETURN_VALUE_IF_NOT(receiptSectionTxnHeader(rec, &txn->dateTime, txn->core.txnType), ERR_OK, ; , ERR_NOK);
...
static int8_t buildDailyRepBodyReceipt(Receipt *rec, const ReceiptData *data) {
    ...
    snprintf(title, sizeof(title), "%s-%s", dtStr, phraseGetDef(PHRASE_SERVICE_NAME));
    RecColumn_t row[] = {
...
int8_t buildReceipt(Receipt *rec,
    ...
    RETURN_VALUE_IF_NULL(rec, ;, ERR_BAD_PARAMETER);
    RETURN_VALUE_IF_NULL(data, ;, ERR_BAD_PARAMETER);
    RETURN_VALUE_IF_NOT(createReceipt(rec), ERR_OK, 
